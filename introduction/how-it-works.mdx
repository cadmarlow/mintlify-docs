...
graph TB
    ...
    C --> E[Firecrawl: Find Prospects]
    C --> F[Hunter: Verify Emails]
    C --> G[GPT-5: Generate Emails]
    C --> H[Resend: Send Emails]
...
### 1. Prospect Discovery (Firecrawl)
...
<Steps>
  ...
  <Step title="tldr.work queries Firecrawl API">
    Firecrawl is a powerful web scraping tool that crawls the web intelligently.
  </Step>
  <Step title="Firecrawl finds matching companies">
...
<Info>
**Why Firecrawl?** It's specifically designed for business data extraction and returns structured, clean data.
</Info>

### 2. Email Enrichment (Hunter.io)

**What happens when you enrich prospect data:**

<Steps>
  <Step title="tldr.work sends company domain to Hunter">
    Example: `bakery-paris.fr`
  </Step>
  <Step title="Hunter finds all email addresses">
    Searches public sources: company website, LinkedIn, directories.
  </Step>
  <Step title="Hunter verifies each email">
...
<Steps>
  ...
  <Step title="Sends via Resend API">
    Resend handles SMTP, SPF, DKIM, bounce handling.
  </Step>
...
<Steps>
  <Step title="tldr.work scrapes your website">
    Uses Firecrawl to extract main content (text, metadata, structure).
  </Step>
...
### 3. Multi-Engine Testing (OpenRouter)
...
<Steps>
  ...
  <Step title="Sends each prompt to 5 AI engines">
    Via OpenRouter API (unified gateway for all LLMs).
  </Step>
...
<Info>
**Why OpenRouter?** It provides access to all major AI models via one API, with consistent formatting and billing.
</Info>
...
### AI & APIs
- **OpenRouter**: Unified gateway to all AI models (GPT, Claude, Gemini, etc.)
...
### Accuracy
- **Prospect data**: 95%+ accuracy (Firecrawl filters out invalid data)
- **Email verification**: 98%+ deliverability (Hunter.io's verification algorithm)
...
<Note>
**Questions?** Check our [Troubleshooting Guide](/guides/troubleshooting) or email support@tldr.work
</Note>
